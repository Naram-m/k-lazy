...
def generate_g(T, d=2, P=2, start_positive=True, var=10, seed=None):
    T, d, P = int(T), int(d), max(1, int(P))
    start_sign = 1 if start_positive else -1
    # base row with ℓ2 = 1: (1/√d) * [1,1,...,1]
    base_row = (1.0 / np.sqrt(d)) * np.ones(d, dtype=float)

    blocks = []
    for i in range(P):
        sign = start_sign if (i % 2 == 0) else -start_sign
        block = sign * np.tile(base_row, (T, 1))
        blocks.append(block)
...
def comparator_sequence(T_phase, d=2, P=10, start_positive=True):
    """
    Returns u_t of shape (T_phase*P, d).
    Each phase uses a constant comparator with ||·||_1 = 1 and
    sign opposite to the phase mean (alternating +, -, +, ...).
    """
...
if __name__ == "__main__":
    os.makedirs("./klazy_results_wc", exist_ok=True)
    ...
    # --- generate sequence ---
    g_t = generate_g(T, d=d, start_positive=start_positive, P=5, seed=42)
    comparator = comparator_sequence(T_phase=T, d=d, P=5, start_positive=start_positive)
    ...
    np.save("./klazy_results_wc/gd_sc_acc.npy", np.array(gd_sc_acc_list))
    np.save("./klazy_results_wc/lgd_sc_acc.npy", np.array(lgd_sc_acc_list))
    np.save("./klazy_results_wc/u_sc_acc.npy", np.array(u_t_sc_acc_list))

    np.save("./klazy_results_wc/gd_hc_acc.npy", np.array(gd_hc_acc_list))
    np.save("./klazy_results_wc/lgd_hc_acc.npy", np.array(lgd_hc_acc_list))
    np.save("./klazy_results_wc/u_hc_acc.npy", np.array(u_t_hc_acc_list))

    # KLAZY variants: suffix by k
    for k in k_values:
        np.save(f"./klazy_results_wc/klazy_sc_acc_k{k}.npy", np.array(klazy_sc_acc_lists[k]))
        np.save(f"./klazy_results_wc/klazy_hc_acc_k{k}.npy", np.array(klazy_hc_acc_lists[k]))
